...
void UVSphere::save(const std::string filename) {
  ...
  // Pushing the top and bottom center points into the positions array for later use
  glm::vec3 top(0.0, 0.0, this->radius);
  positions.push_back(top);
  glm::vec3 bottom(0.0, 0.0, (-1 * this->radius));
  ...
  for (unsigned int latitudeIndex = 0; latitudeIndex < this->numHorizontalSegments; latitudeIndex++) {
    ...
    for (unsigned int longitudeIndex = 0; longitudeIndex < this->numVerticalSegments; longitudeIndex++) {
      ...
      // generate one vertex
      float x = this->radius * cos(theta);  // Parametric equation for x value
      float y = this->radius * sin(theta);  // Parametric equation for y value
      float z = this->radius;               // This is for the depth of the cylinder
      
      // If the latitude is greater than 0, deal with the bottom circle here
      if (latitudeIndex > 0) {
          z = -1 * z;
      }
      
      // Takes values from above and puts them into the positions array
      glm::vec3 pos(x, y, z);
  ...
  // handle the last column special case
    unsigned int firstCol = 2;
    unsigned int lastCol = this->numHorizontalSegments - 1;

    unsigned int topp =  0;
    unsigned int bott = 1;
    for (row = 1; row < this->numVerticalSegments; row++) {
      // Here is where the sides of the cylinder are created
      unsigned int a = ((row-1) * this->numVerticalSegments)%numVerticalSegments+ (row-1) + firstCol;
      unsigned int b = ((row-1) * this->numVerticalSegments)%numVerticalSegments+ (row-1) + 3;
      unsigned int c = ((row-1) * this->numVerticalSegments)%numVerticalSegments+ (row-1) + (numVerticalSegments+firstCol);
      unsigned int d = ((row-1) * this->numVerticalSegments)%numVerticalSegments+ (row-1) + (numVerticalSegments+firstCol+1);
      std::cout << "a: " << a << " b: " << b << " c: " << c << " d: " << d << std::endl;

      vertexIndices.push_back(topp);//top triangle
      vertexIndices.push_back(a);
      vertexIndices.push_back(b);

      vertexIndices.push_back(c);//bottom triangle
      vertexIndices.push_back(bott);
      vertexIndices.push_back(d);

      vertexIndices.push_back(a);//side half rectangle
      vertexIndices.push_back(b);
      vertexIndices.push_back(c);

      vertexIndices.push_back(d);//other half rectangle
      vertexIndices.push_back(c);
      vertexIndices.push_back(b);
         
      // This is used to fix a problem with the last bit of connectivity
      if (row == this->numVerticalSegments-1) {
        unsigned int p1 = ((0) * this->numVerticalSegments)%numVerticalSegments+ firstCol;
        unsigned int p2 = ((0) * this->numVerticalSegments)%numVerticalSegments+  (numVerticalSegments+firstCol-1);
        unsigned int p3 = ((0) * this->numVerticalSegments)%numVerticalSegments+  (numVerticalSegments+firstCol);
        std::cout << "p1: " << p1 << " p2: " << p2 << " p3: " << p3 << std::endl;
        vertexIndices.push_back(topp);//top triangle
        vertexIndices.push_back(p2);
        vertexIndices.push_back(p1);

        vertexIndices.push_back(d);//bottom triangle
        vertexIndices.push_back(bott);
        vertexIndices.push_back(p3);

        vertexIndices.push_back(p2);//side half rectangle
        vertexIndices.push_back(p3);
        vertexIndices.push_back(d);

        vertexIndices.push_back(p3);//other half rectangle
        vertexIndices.push_back(p1);
        vertexIndices.push_back(p2);
